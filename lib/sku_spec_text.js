...
const DUPLICATE_SKU_PROPERTY_NAME_FALLBACKS = Object.freeze([
  'Quantity',
  'Default',
  'Specification',
  'Style',
...
function isQuantityLikeSkuProperty(property = {}) {
  const attrName = normalizeSpecTranslationTextSafe(property && property.attrName);
  const attrValues = (Array.isArray(property && property.attrValueList) ? property.attrValueList : [])
    .map((value) => normalizeSpecTranslationTextSafe(value && value.attrValue))
    .filter(Boolean);
  const combined = [attrName, ...attrValues].join(' ');
  const hasEnglishQuantityUnit = /\d+(?:\.\d+)?\s*(?:ml|g|kg|oz|lb|pcs?|pieces?|packs?|sets?|pairs?|count|ct)\b/i.test(combined);
  const hasChineseQuantityUnit = /\d+(?:\.\d+)?\s*(?:瓶|支|个|片|包|盒)/.test(combined);

  return /(?:quantity|qty|count|net\s*content|capacity|amount|数量|净含量|容量|规格)/i.test(combined)
    || hasEnglishQuantityUnit
    || hasChineseQuantityUnit;
}

function buildNumberedSkuPropertyName(baseName = 'Option', index = 2) {
  const suffix = ` ${index}`;
  const base = sanitizeSkuSpecText(baseName, Math.max(1, SPEC_ATTR_NAME_MAX_LENGTH - suffix.length))
    || 'Option';
  return sanitizeSkuSpecText(`${base}${suffix}`, SPEC_ATTR_NAME_MAX_LENGTH)
    || `${base}${suffix}`.slice(0, SPEC_ATTR_NAME_MAX_LENGTH).trim();
}

function pickUniqueSkuPropertyName(property = {}, usedNameKeys = new Set(), duplicateIndex = 2) {
  const candidates = [
    ...(isQuantityLikeSkuProperty(property) ? ['Quantity'] : []),
    ...DUPLICATE_SKU_PROPERTY_NAME_FALLBACKS,
  ];

  for (const candidate of candidates) {
    const attrName = sanitizeSkuSpecText(candidate, SPEC_ATTR_NAME_MAX_LENGTH);
    const key = normalizeSkuSpecDuplicateKey(attrName);
    if (attrName && key && !usedNameKeys.has(key)) {
      return attrName;
    }
  }

  let index = duplicateIndex;
  while (index < duplicateIndex + 100) {
    const attrName = buildNumberedSkuPropertyName('Option', index);
    const key = normalizeSkuSpecDuplicateKey(attrName);
    if (attrName && key && !usedNameKeys.has(key)) {
      return attrName;
    }
  ...
  return buildNumberedSkuPropertyName('Option', duplicateIndex);
}

function ensureUniqueSkuPropertyNames(skuPropertyList = []) {
  const usedNameKeys = new Set();
  let duplicateIndex = 2;

  return (Array.isArray(skuPropertyList) ? skuPropertyList : []).map((property) => {
    const attrName = sanitizeSkuSpecText(property && property.attrName, SPEC_ATTR_NAME_MAX_LENGTH)
      || DEFAULT_SINGLE_SPEC_ATTR_NAME;
    const attrNameKey = normalizeSkuSpecDuplicateKey(attrName) || '__empty__';

    if (!usedNameKeys.has(attrNameKey)) {
      usedNameKeys.add(attrNameKey);
      return {
        ...property,
        attrName,
      };
    ...
    const replacementName = pickUniqueSkuPropertyName(property, usedNameKeys, duplicateIndex);
    const replacementKey = normalizeSkuSpecDuplicateKey(replacementName) || `__duplicate_${duplicateIndex}__`;
    duplicateIndex += 1;
    usedNameKeys.add(replacementKey);

    return {
      ...property,
      attrName: replacementName,
    };
  });
}

function ensureUniqueSkuPropertyValueNames(skuPropertyList = []) {
  return ensureUniqueSkuPropertyNames(skuPropertyList).map((property) => {
