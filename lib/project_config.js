...
const PROJECT_CONFIG_SCHEMA = [
  ...
  },
  {
    key: 'ai',
    title: 'AI 模型服务',
    description: '配置默认使用模型，以及各个 AI 大模型服务的版本、接口地址和 Key。',
    groups: [
      {
        key: 'defaultModels',
        title: '默认使用模型',
      ...
      },
      {
        key: 'deepseek',
        title: 'DeepSeek',
        description: '配置 DeepSeek 文字模型服务。',
        fields: [
...
function sectionProjectConfigFields(section = {}) {
  ...
  const groupFields = Array.isArray(section.groups)
    ...
    : [];
  return directFields.concat(groupFields);
}

function flattenProjectConfigFields() {
  return PROJECT_CONFIG_SCHEMA.flatMap((section) => (
    sectionProjectConfigFields(section).map((field) => ({
      ...field,
...
function getProjectConfig(options = {}, context = createContext()) {
  ...
  return {
    ...
    sections: PROJECT_CONFIG_SCHEMA.map((section) => ({
      ...
      fields: sectionProjectConfigFields(section).map((field) => serializeProjectConfigField(field, env, { includeLocalEnv })),
      groups: Array.isArray(section.groups)
        ? section.groups.map((group) => ({
          key: group.key,
          title: group.title,
          description: group.description || '',
          fields: (Array.isArray(group.fields) ? group.fields : []).map((field) => serializeProjectConfigField(field, env, { includeLocalEnv })),
        }))
        : [],
    })),
