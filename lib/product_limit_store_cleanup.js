...
const STORE_DISPLAY_COUNTRY_TEXT_PATTERN = /^(.*?)\s+(菲律宾|马来|泰国|越南|新加坡|印尼)\s*$/;
...
function normalizeFailureReasonForMatch(reason = '') {
  return String(reason || '').replace(/\s+/g, '').replace(/[,，]/g, '');
}

function matchesProductLimitFailureReason(reason = '') {
  const normalizedReason = normalizeFailureReasonForMatch(reason);
  return PRODUCT_LIMIT_REASON_PATTERNS.every((pattern) => (
    normalizedReason.includes(normalizeFailureReasonForMatch(pattern))
  ));
}
...
function parseUnpublishMetricCount(text, labels = []) {
  ...
  for (const label of labels) {
    ...
    const patterns = [
      new RegExp(`${escapedLabel}\\s*[：:]?\\s*[（(]?\\s*(\\d+)`),
      new RegExp(`${escapedLabel}\\D{0,12}(\\d+)\\s*(?:个|条|项|件)?`),
    ];
...
function parseUnpublishOperationResultText(text = '') {
  return {
    totalCount: parseUnpublishMetricCount(text, ['总计', '合计', '共']),
    successCount: parseUnpublishMetricCount(text, ['成功', '已成功']),
    failureCount: parseUnpublishMetricCount(text, ['失败', '未成功']),
    unfinishedCount: parseUnpublishMetricCount(text, ['未完成', '未处理']),
  };
}
...
function normalizeStoreName(value = '') {
  return String(value || '')
    ...
    .replace(/\s+(菲律宾|马来|泰国|越南|新加坡|印尼)\s*$/, '')
    .trim();
}
...
function normalizeStoreComparableText(value = '') {
  return normalizeStoreSearchText(value)
    .replace(/\s*-\s*(菲律宾|马来|泰国|越南|新加坡|印尼)/g, '-$1')
    .replace(/\s+(菲律宾|马来|泰国|越南|新加坡|印尼)/g, '-$1')
    .trim();
}

function hasExplicitStoreOptionText(value = '') {
  const text = String(value || '').replace(/\s+/g, ' ').trim();
  return STORE_OPTION_TEXT_PATTERN.test(text) || STORE_DISPLAY_COUNTRY_TEXT_PATTERN.test(text);
}
...
function normalizeStoreOptionText(value = '') {
  ...
  const suffixMatch = normalizedValue.match(STORE_OPTION_TEXT_PATTERN)
    || normalizedValue.match(STORE_DISPLAY_COUNTRY_TEXT_PATTERN);
...
function resolveStoreTarget(store = {}, explicitStoreOptionText = '') {
  ...
  const storeSearchText = normalizeStoreSearchText(
    rawStore.storeSearchText
      || explicitStoreOptionText
      || rawStore.storeOptionText
      || rawStore.optionText
      || rawStore.storeName
      || rawStore.shopName
      || '',
  ...
  const storeName = normalizeStoreSearchText(rawStore.storeName || rawStore.shopName || storeSearchText || storeOptionText || optionCandidate);
...
function formatStoreLogName(store = {}) {
  const target = resolveStoreTarget(store);
  return target.storeSearchText || target.storeName || target.storeOptionText || '未知店铺';
}
  ...
  return (Array.isArray(stores) ? stores : [])
    .map((store) => {
      ...
      const target = resolveStoreTarget({
        ...
        storeSearchText: rawStore.storeSearchText || candidate,
...
function dedupeLimitStoreRecords(records = []) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    const target = resolveStoreTarget({
      ...
      storeSearchText: (record || {}).storeSearchText,
      storeOptionText: (record || {}).storeOptionText,
    });
    if (!target.storeSearchText && !target.storeName) {
      continue;
    }

    const dedupeKey = target.storeSearchText || target.storeName;
    countsByStoreName.set(dedupeKey, (countsByStoreName.get(dedupeKey) || 0) + 1);
  }

  return [...countsByStoreName.entries()].map(([storeSearchText, failureCount]) => ({
    storeName: storeSearchText,
    storeSearchText,
    failureCount,
  }));
}
...
function summarizeUnmatchedFailureRecords(records = [], limit = 5) {
  const safeLimit = Math.max(1, Number.parseInt(limit, 10) || 5);
  return (Array.isArray(records) ? records : [])
    .filter((record) => {
      const reason = (record || {}).reason || (record || {}).failureReason || (record || {}).rawText || '';
      const target = resolveStoreTarget({
        storeName: (record || {}).storeName || (record || {}).shopName,
        storeSearchText: (record || {}).storeSearchText,
        storeOptionText: (record || {}).storeOptionText,
      });
      return reason && (!matchesProductLimitFailureReason(reason) || !(target.storeSearchText || target.storeName));
    })
    .map((record) => {
      const reason = (record || {}).reason || (record || {}).failureReason || (record || {}).rawText || '';
      const reasonPreview = String(reason || '').replace(/\s+/g, ' ').trim().slice(0, 180);
      const target = resolveStoreTarget({
        storeName: (record || {}).storeName || (record || {}).shopName,
        storeSearchText: (record || {}).storeSearchText,
        storeOptionText: (record || {}).storeOptionText,
      });
      const storeName = target.storeName || target.storeSearchText;
      return {
        storeName,
        storeOptionText: target.storeOptionText || '',
        storeSearchText: target.storeSearchText || '',
        reasonPreview: matchesProductLimitFailureReason(reason) && !(target.storeSearchText || target.storeName)
          ? `已匹配上限原因，但没有解析到店铺名：${reasonPreview}`
          : reasonPreview,
      };
    })
    .filter((record) => record.reasonPreview)
    .slice(0, safeLimit);
}
...
function buildLimitStoreSummary({
  ...
  return {
    ...
    unmatchedFailureRecords: Array.isArray(unmatchedFailureRecords) ? unmatchedFailureRecords : [],
    ...
    skippedStores: safeResults
      ...
      .map((result) => {
        ...
        if (Object.prototype.hasOwnProperty.call(result, 'storeSearchText')) {
          skippedStore.storeSearchText = result.storeSearchText || '';
        }
        return skippedStore;
      }),
...
async function verifySelectedStoreTag(page, target, timeout = 10000) {
  const storeSearchText = target.storeSearchText || target.storeOptionText || target.storeName;
  try {
...
async function extractFailurePageRecords(page) {
  return page.evaluate(() => {
    const storeOptionPattern = /^(.*?)\s*-\s*(菲律宾|马来|泰国|越南|新加坡|印尼)\s*$/;
    const storeDisplayCountryPattern = /^(.*?)\s+(菲律宾|马来|泰国|越南|新加坡|印尼)\s*$/;
    const isVisible = (element) => {
      if (!element) {
        return false;
      }
    ...
    const collectAttributeText = (root) => Array.from(root.querySelectorAll('[title], [aria-label], [data-title]'))
      .flatMap((element) => [
      ...
      .map(normalize)
      .filter(Boolean);
    const normalizeStoreNameText = (value) => String(value || '')
      .replace(/\s+/g, ' ')
      .trim()
      .replace(/\s*-\s*(菲律宾|马来|泰国|越南|新加坡|印尼)\s*$/, '')
      .replace(/\s+(菲律宾|马来|泰国|越南|新加坡|印尼)\s*$/, '')
    ...
    const looksLikeStoreName = (value) => {
      ...
      if (!(storeOptionPattern.test(text)
        || storeDisplayCountryPattern.test(text)
        || /\b(PH|MY|TH|VN|SG|IDN|ID)\b\s*$/i.test(text))) {
        return false;
      }
      if (/发布失败|商店试用期|最多只能|产品ID|类目|主账号|子账号|\d{4}-\d{2}-\d{2}|编辑|删除|记录/.test(text)) {
        return false;
      }
      const countryMatch = text.match(storeOptionPattern) || text.match(storeDisplayCountryPattern);
      const storeNameText = countryMatch ? normalizeStoreNameText(text) : text;
      return /^[\w\s&'.-]+$/i.test(storeNameText);
    };

    return Array.from(document.querySelectorAll('tr, .ant-table-row, .el-table__row, [role="row"]'))
      .filter(isVisible)
      .map((row) => {
        const visibleText = String(row.innerText || row.textContent || '').trim();
        const rawText = [
          visibleText,
          ...collectAttributeText(row),
        ].filter(Boolean).join('\n').trim();
        ...
        const cells = Array.from(row.querySelectorAll('td, [role="cell"]'))
          ...
          .map((cell) => [
            ...
            ...collectAttributeText(cell),
          ].filter(Boolean).join(' '))
          .filter(Boolean);
        const storeCandidate = [...cells, ...lines].reverse().find(looksLikeStoreName) || '';
        return {
          storeName: storeCandidate,
          storeSearchText: storeCandidate,
          storeOptionText: '',
          reason: rawText,
      ...
      .filter((record) => /发布失败|商店试用期|最多只能/.test(record.rawText)
        || looksLikeStoreName(record.storeName)
        || storeOptionPattern.test(record.storeOptionText));
  });
}

async function waitForFailurePageRecordsSettled(page, {
  timeout = 8000,
  interval = 500,
  stableRounds = 2,
} = {}) {
  ...
  while (Date.now() < deadline) {
    const records = await extractFailurePageRecords(page).catch(() => []);
    const matchedCount = buildMatchedLimitStoreRecords(records).length;
    const signature = records
      .map((record) => [
        record.storeName || record.shopName || '',
        String(record.rawText || record.reason || record.failureReason || '').replace(/\s+/g, ' ').slice(0, 240),
      ].join('|'))
      .join('\n');

    if (records.length > bestRecords.length || matchedCount > bestMatchedCount) {
      bestRecords = records;
      bestMatchedCount = matchedCount;
    }

    if (records.length > 0 && signature && signature === lastSignature) {
      stableCount += 1;
    } else {
      stableCount = 0;
    ...
    if (records.length > 0 && stableCount >= stableRounds) {
      return records;
    }
...
async function scanProductLimitFailureStores(page, { maxPages = 5 } = {}) {
  ...
  for (let pageIndex = 0; pageIndex < safeMaxPages; pageIndex += 1) {
    const pageRecords = await waitForFailurePageRecordsSettled(page);
    records.push(...pageRecords);
    const matchedStores = buildMatchedLimitStoreRecords(records);
    const unmatchedFailureRecords = summarizeUnmatchedFailureRecords(records);
    emitProgress({
      phase: 'scan',
      phaseLabel: `已扫描发布失败第 ${pageIndex + 1} 页：读取 ${records.length} 条失败记录，命中 ${matchedStores.length} 个店铺`,
  ...
  return {
    ...
    unmatchedFailureRecords: summarizeUnmatchedFailureRecords(records),
  };
}
...
async function verifySafeProductPage(page, store, explicitStoreOptionText = '') {
  const target = resolveStoreTarget(store, explicitStoreOptionText);
  const storeSearchText = target.storeSearchText || target.storeOptionText || target.storeName;
  const storeLogName = formatStoreLogName(target);
  const text = await page.evaluate(() => String(document.body.innerText || document.body.textContent || ''));
  const comparableText = normalizeStoreComparableText(text);
  const comparableStoreSearchText = normalizeStoreComparableText(storeSearchText);
  const storeVisible = Boolean(comparableStoreSearchText) && comparableText.includes(comparableStoreSearchText);
  const currentPageSize = await readCurrentProductPageSize(page);
  ...
  if (!safe) {
    ...
    emitProgress({
      ...
      storeOptionText: target.storeOptionText,
      storeSearchText: target.storeSearchText,
      phaseLabel: `${storeLogName}：安全校验失败：${safeCheckDetails || '未知原因'}，已跳过下架`,
    });
  }
...
async function readUnpublishOperationResultText(page) {
  return page.evaluate(() => {
    const isVisible = (element) => {
      if (!element) {
        return false;
    ...
    const normalize = (value) => String(value || '').replace(/\s+/g, ' ').trim();
    const resultContainers = Array.from(document.querySelectorAll([
      '.ant-modal',
      '.ant-modal-content',
      '.ant-drawer',
      '.ant-popover',
      ...
      .filter(isVisible)
      .map((element) => normalize(element.innerText || element.textContent))
      .filter((text) => /下架|总计|成功|失败|未完成/.test(text));

    if (resultContainers.length > 0) {
      return resultContainers[0];
    }

    const bodyText = normalize(document.body && (document.body.innerText || document.body.textContent));
    return /下架|总计|成功|失败|未完成/.test(bodyText) ? bodyText : '';
  });
}

function hasUnpublishOperationSignal(result, rawText = '') {
  return [
    result.totalCount,
    result.successCount,
    result.failureCount,
    ...
    || /下架成功|操作成功|成功|失败/.test(rawText);
}

function isUnpublishOperationTerminal(result, rawText = '') {
  if (Number.isFinite(Number(result.unfinishedCount)) && Number(result.unfinishedCount) > 0) {
    return false;
  }

  return Number.isFinite(Number(result.failureCount))
    || Number.isFinite(Number(result.successCount))
    || /下架成功|操作成功|成功|失败/.test(rawText);
}

async function waitForUnpublishOperationResult(page, timeout = 60000) {
  const deadline = Date.now() + timeout;
  let bestResult = null;

  while (Date.now() < deadline) {
    const rawText = await readUnpublishOperationResultText(page);
    const parsedResult = parseUnpublishOperationResultText(rawText);
    const result = {
      ...parsedResult,
      rawText,
    };

    if (hasUnpublishOperationSignal(result, rawText)) {
      bestResult = result;
      if (isUnpublishOperationTerminal(result, rawText)) {
        return result;
      }
    }
  ...
  const rawText = await readUnpublishOperationResultText(page).catch(() => '');
  return {
    ...parseUnpublishOperationResultText(rawText),
    rawText,
  };
}
...
async function unpublishCurrentPageProducts(page) {
  ...
  const operationResult = await waitForUnpublishOperationResult(page);
...
function getSuccessfulUnpublishCount(result = {}) {
  if (Number.isFinite(Number(result.successCount))) {
    return Number(result.successCount);
  }

  if (
    Number.isFinite(Number(result.selectedProductCount))
    && Number.isFinite(Number(result.failureCount))
  ) {
    return Math.max(0, Number(result.selectedProductCount) - Number(result.failureCount));
  }

  return Number.isFinite(Number(result.selectedProductCount))
    ? Number(result.selectedProductCount)
    : 0;
}

function hasUnpublishOperationFailures(result = {}) {
  return (
    Number.isFinite(Number(result.failureCount)) && Number(result.failureCount) > 0
  ) || (
    Number.isFinite(Number(result.unfinishedCount)) && Number(result.unfinishedCount) > 0
  ) || (
    !Number.isFinite(Number(result.failureCount))
    && /失败|错误|异常/.test(result.rawText || '')
  );
}
  ...
  while (cleanupRound < MAX_ZERO_SALES_CLEANUP_ROUNDS) {
    ...
    if (!Number.isFinite(Number(zeroSalesProductCount))) {
      const reason = totalUnpublishedCount > 0
        ? `${storeLogName}：已下架 ${totalUnpublishedCount} 个后无法确认零销量商品数，已停止`
        : `${storeLogName}：无法确认零销量商品数，已跳过下架`;
      emitProgress({
        phase: 'skip',
        storeName: target.storeName,
        storeOptionText: target.storeOptionText,
    ...
    if (
      ...
      emitProgress({
        ...
        zeroSalesProductCount,
        retainCount: zeroSalesRetainCount,
        phaseLabel: `${storeLogName}：下架后零销量商品数量暂未减少，等待重新确认（当前 ${zeroSalesProductCount} 个，上一轮 ${previousZeroSalesProductCount} 个）`,
        unpublishedCount: totalUnpublishedCount,
      });
      await sleep(3000);
      await openFilteredZeroSalesProducts(page, target);
      ...
      if (
        ...
        const error = new Error(
          `${storeLogName}：下架后零销量商品数量没有减少，当前 ${zeroSalesProductCount || '未确认'} 个，上一轮 ${previousZeroSalesProductCount} 个，保留数量 ${zeroSalesRetainCount} 个`,
        );
    ...
    if (shouldSkipUnpublishByZeroSalesCount(zeroSalesProductCount, zeroSalesRetainCount)) {
      if (totalUnpublishedCount > 0) {
        emitProgress({
          ...
          zeroSalesProductCount,
          retainCount: zeroSalesRetainCount,
          phaseLabel: `${storeLogName}：已完成下架，当前零销量商品 ${zeroSalesProductCount} 个，不超过保留数量 ${zeroSalesRetainCount} 个`,
          unpublishedCount: totalUnpublishedCount,
        });
        return {
          storeName: target.storeName,
      ...
      const reason = `${storeLogName}：零销量商品不超过 ${zeroSalesRetainCount} 个（当前 ${zeroSalesProductCount} 个），已跳过下架`;
      emitProgress({
        phase: 'skip',
        storeName: target.storeName,
        storeOptionText: target.storeOptionText,
    ...
    const unpublishResult = await unpublishCurrentPageProducts(page);
    totalUnpublishedCount += getSuccessfulUnpublishCount(unpublishResult);
    cleanupRound += 1;

    if (hasUnpublishOperationFailures(unpublishResult)) {
      const failureCountText = Number.isFinite(Number(unpublishResult.failureCount))
        ? `失败 ${unpublishResult.failureCount} 个`
        : '';
      const unfinishedCountText = Number.isFinite(Number(unpublishResult.unfinishedCount))
        ? `未完成 ${unpublishResult.unfinishedCount} 个`
        : '';
      const resultText = [failureCountText, unfinishedCountText].filter(Boolean).join('，');
      emitProgress({
        phase: 'cleanup',
        storeName: target.storeName,
        storeOptionText: target.storeOptionText,
        ...
        zeroSalesProductCount,
        retainCount: zeroSalesRetainCount,
        selectedProductCount: unpublishResult.selectedProductCount,
        unpublishResult,
        unpublishedCount: totalUnpublishedCount,
        cleanupRound,
        phaseLabel: `${storeLogName}：下架结果仍有${resultText || '失败'}，会重新筛选当前店铺继续下架`,
      });
      await sleep(1500);
      continue;
    }
...
async function runProductLimitStoreCleanup({
  ...
  if (Array.isArray(stores) && stores.length > 0) {
    ...
    unmatchedFailureRecords = scanResult.unmatchedFailureRecords;
  }
